        ...
        if (source && exports.isObject(source)) {
          for (var name in source) {
            ...
            if (
              deep &&
              copy &&
              (exports.isPlainObject(copy) || (copyIsArray = exports.isArray(copy)))
            ) {
              if (copyIsArray) {
                copyIsArray = false
...
;(function() {
  ...
  Calendar.prototype = {
    ...
    refreshData: function(dateObj, callback) {
      ...
      if (self.nowYear == self.curYear && parseInt(self.nowMonth) == self.curMonth) {
        // debugger
      }
      // 获取星期
      ...
      if (dayCount === '28') {
        ...
        for (
          var index = nowXingQiJi + 1, indexindex = 1;
          index < 28 + nowXingQiJi + 1;
          index++, indexindex++
        ) {
          ...
          tmpInfo.push({
            ...
            // isforbid: '1' // 当前月可点击
            isforbid: indexindex < self.curDay ? '0' : '1' //当前月可点击
          })
      ...
      if (dayCount == '30') {
        ...
        for (
          ...
          fileInfo[lunar] = self._getLunar(indexindex)
          //存储当前月数据
          // console.log('---indexindex---:', indexindex)
          // debugger
          tmpInfo.push({
            day: indexindex, //日
            lunar: self._getLunar(indexindex), //农历
      ...
      for (var i = 0, len = dataObj.tmpInfo.length; i < len; i++) {
        ...
        if (!templData) {
          ...
          if (value.date == curr) {
            ...
            if (value.isSelected) {
              template =
                '<div class="em-calendar-item em-calendar-active  isforbid{{isforbid}} tip{{tip}}" date="{{date}}"><span class="day">{{day}}</span><p class="lunar">今天</p><span class="dot dot-type1"></span></div>'
            } else {
              template =
                '<div class="em-calendar-item em-calendar-active  isforbid{{isforbid}} tip{{tip}}" date="{{date}}"><span class="day">{{day}}</span><p class="lunar">今天</p></div>'
            }
          } else {
            if (value.isSelected) {
              // 个性化和业务贴近
    ...
    CreateDOMFactory: function(callback) {
      ...
      // 头部样式
      self.HEADER_BAR =
        '<div class="em-calendar-container"><div class="em-week">\
    ...
    setActiveStyle: function(currMonthIndex) {
      let self = this
      if (self.tom(currMonthIndex) !== self.curMonth) {
        var chooseDom = document.querySelector('.swiper-slide-active .boqii-calendar-active')
        if (chooseDom) {
          chooseDom.classList.remove('boqii-calendar-active')
        }
        document
          .querySelector('.swiper-slide-active .isforbid1')
          .classList.add('boqii-calendar-active')
      }
    },
    /**
     * 增加事件，包括
    ...
    _addEvent: function() {
      ...
      var disableSwiperCallback = self.options.disableSwiperCallback
      // 记录上一月和上一年的索引
      ...
      self.mySwiper = new Swiper('.swiper-container', {
        ...
        on: {
          tap: function(e) {
            ...
            if (!_this.classList.contains('isforbid0')) {
              if (_this.getAttribute('date')) {
                _this.classList.add('boqii-calendar-active')
                self.sibling(_this, function(el) {
                  el.classList.remove('boqii-calendar-active')
                })
              }
              // 点击回调
              onItemClick &&
          ...
          slideNextTransitionStart: function() {
            // if (self.nowMonth > self.curMonth + 1) return
            debugger
            let swiper = this
            count++
            if (count == '1') {
            ...
            if (self.curYear == nextYear) {
              if (Number(nextMonth) - Number(self.curMonth) > 1) {
                // 如果往后滑动时后一个月非当前月，重新渲染1号样式
                self.setActiveStyle(currMonthIndex)
                disableSwiperCallback('next')
                return
              }
            } else if (nextYear > self.curYear) {
              if (Number(nextMonth) + 12 - Number(self.curMonth) > 1) {
                // 如果往后滑动时后一个月非当前月，重新渲染1号样式
                self.setActiveStyle(currMonthIndex)
                disableSwiperCallback('next')
                return
          ...
          slidePrevTransitionStart: function() {
            ...
            if(count == '1') {
              currYearIndex = parseInt(self.curYear)
            } else {
              if (currMonthIndex == 0) {
                currYearIndex = parseInt(currYearIndex) - parseInt(1)
                currMonthIndex = 12
            ...
            // 刷新日历
            if (self.curYear == nextYear) {
              if (Number(self.curMonth) - Number(preMonth) > 0) {
                disableSwiperCallback('pre')
                return
              }
            } else if (nextYear > self.curYear) {
              disableSwiperCallback('pre')
    ...
    addActiveStyleFordate: function(dateStr) {
      var self = this
      // 给前后月的点击日期加选中标记
      var clickactives = document
        .querySelector('.swiper-slide-active')
        .querySelectorAll('.boqii-calendar-item')
      for (var i = 0; i < clickactives.length; i++) {
        if (clickactives[i].getAttribute('date') == dateStr) {
          clickactives[i].classList.add('boqii-calendar-active')
    ...
    _e2c: function() {
      ...
      total =
        ...
        self.madd[self.TheDate.getMonth()] +
