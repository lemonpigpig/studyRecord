<template lang="html">
  <div class="gz-datetime-picker">
    <gz-mask class="publish-mask" :show="show" @click="cancle"></gz-mask>
    <div class="gz-datetime-panel">
        <gz-picker
          :slots="dateSlots"
          @change="onChange"
          :visible-item-count="visibleItemCount"
          class="gz-datetime-picker"
          ref="picker" :showToolbar="showToolbar" :itemHeight="itemHeight">
            <slot>
...
<script>
import GzPicker from '../picker/picker.vue';
...
export default {
    name: 'gz-datetime-picker',
    components: {
        [GzPicker.name]: GzPicker
    },
    props: {
        itemHeight: {
            type: Number,
        ...
        endDate: {
            type: Date,
            default() {
                return new Date(new Date().getFullYear() + delta, 11, 31);
            }
        },
        startHour: {
            type: Number,
    ...
    mounted() {
        ...
        if (!this.value) {
            if (this.type.indexOf('date') > -1) {
                ...
                const tempStr = (`0${this.startHour}`).slice(-2);
                this.currentValue = `${tempStr}:00`;
            }
        }
        this.generateSlots();
    },
    ...
    computed: {
        rims() {
            ...
            result = {
                ...
                month: [1, 12],
                date: [1, this.getMonthEndDay(this.getYear(this.currentValue),
    ...
    methods: {
        ...
        getTrueValue(formattedValueA) {
            let formattedValue = formattedValueA;
        ...
        getValue(values) {
            let value;
            if (this.type === 'time') {
                value = values.map(v => (`0${this.getTrueValue(v)}`).slice(-2)).join(':');
            } else {
